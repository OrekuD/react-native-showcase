...
import { CONTENT_EDGE_INSET, CONTENT_ENTERING, CONTENT_EXITING, MenuContext, styles, type MenuContentProps, type MenuContextValue } from "./menuShared";

type MenuCustomContentProps = Omit<MenuContentProps, "align" | "sideOffset" | "size"> & {
  align: NonNullable<MenuContentProps["align"]>;
  context: MenuContextValue;
  sideOffset: number;
  size: NonNullable<MenuContentProps["size"]>;
};

export function MenuCustomContent({ align, children, context, sideOffset, size }: MenuCustomContentProps) {
  const { theme } = context;
  ...
  const contentContext = useMemo<MenuContextValue>(() => ({ ...context, size }), [context, size]);
  return (
    <DropdownMenuPrimitive.Portal>
      <DropdownMenuPrimitive.Overlay asChild><Pressable accessibilityLabel="Dismiss menu" accessibilityRole="button" style={StyleSheet.absoluteFill} /></DropdownMenuPrimitive.Overlay>
      <DropdownMenuPrimitive.Content align={align} avoidCollisions insets={{ bottom: CONTENT_EDGE_INSET, left: CONTENT_EDGE_INSET, right: CONTENT_EDGE_INSET, top: CONTENT_EDGE_INSET }} side={side} sideOffset={sideOffset}>
        <Animated.View entering={CONTENT_ENTERING} exiting={CONTENT_EXITING}>
          <FastSquircleView cornerSmoothing={theme.cornerSmoothing} style={[styles.panel, size === "compact" && styles.panelCompact, { borderRadius: theme.borderRadius, width: Math.min(320, Math.max(0, viewportWidth - 32)) }, createOutsetShadow({ blurRadius: 24, color: theme.shadowColor, elevation: 10, offsetY: 16, opacity: 0.14 })]}>
            ...
            <MenuContext.Provider value={contentContext}><View style={[styles.content, size === "compact" && styles.contentCompact]}>{children}</View></MenuContext.Provider>
          </FastSquircleView>
        </Animated.View>
